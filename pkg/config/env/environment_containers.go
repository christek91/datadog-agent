...
// isReachable reports whether a socket/pipe probed via socket.IsAvailable should be
// treated as a working endpoint for feature detection purposes. On non-Windows
// platforms, any non-permission dial error (e.g. connection refused on a stale
// socket) still counts as reachable, so the health check module for the associated
// issue gets wired up even though the endpoint is currently broken. On Windows,
// socket.IsAvailable already surfaces every named-pipe dial error (busy, timeout,
// etc.) this way, so applying the same compatibility treatment would wrongly select
// a pipe that is only transiently unreachable; keep the stricter "no error at all"
// requirement there.
