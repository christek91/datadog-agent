...
func TestProcessCheckRetainsScrubbedProcesses(t *testing.T) {
	for _, tc := range []struct {
		name              string
		enabled, stripAll bool
		wantArgs          []string
	}{
		{"scrubbed", true, false, []string{"parent", "--password=********"}},
		{"disabled", false, false, []string{"parent", "--password=secret"}},
		{"strip all", true, true, []string{"parent"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			check, probe, wmeta := processCheckWithMocks(t)
			check.scrubber.Enabled = tc.enabled
			check.scrubber.StripAllArguments = tc.stripAll
			process := makeProcessWithCreateTime(100, "parent --password=secret", 1000)
			process.Ppid = 42
			procs := map[int32]*procutil.Process{process.Pid: process}
			mockProcesses(check.WLMProcessCollectionEnabled(), probe, wmeta, procs, procsToStats(procs))

			for run := 0; run < 2; run++ {
				result, err := check.run(0, false)
				require.NoError(t, err)
				retained := check.lastProcs[process.Pid]
				require.NotNil(t, retained)
				assert.Equal(t, process.Ppid, retained.Ppid)
				assert.Same(t, process.Stats, retained.Stats)
				assert.Equal(t, tc.wantArgs, retained.Cmdline, "retained commands respect scrubbing settings, even on the first poll")
				if run == 0 {
					assert.Equal(t, CombinedRunResult{}, result)
				} else {
					require.Len(t, result.Payloads(), 1)
					payload := result.Payloads()[0].(*model.CollectorProc)
					require.Len(t, payload.Processes, 1)
					assert.Equal(t, tc.wantArgs, payload.Processes[0].Command.Args)
				}
				check.clock.(*clock.Mock).Add(10 * time.Second)
			}
		})
	}
}
