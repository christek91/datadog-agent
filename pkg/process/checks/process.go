...
// retainProcesses scrubs command lines before retaining the collected processes.
// Call it after extraction and formatting, which need the original command lines.
func (p *ProcessCheck) retainProcesses(procs map[int32]*procutil.Process) {
	for _, proc := range procs {
		if proc != nil {
			proc.Cmdline = p.scrubber.ScrubProcessCommand(proc)
		}
	}
	p.lastProcs = procs
}
...
func fmtProcesses(
	...
	for _, fp := range procs {
		...
		command.Args = scrubber.ScrubProcessCommand(fp)
